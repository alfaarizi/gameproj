...
func _show_text(dialogue_item_idx: int) -> void:	
	...
	#### BEGIN: REMOVE THIS LATER, ONLY FOR DEMONSTRATION
	#if curr_character == npc:
		#const therapist_texture = preload("res://assets/Therapist.png")
		#curr_character.body.texture = therapist_texture
		#curr_character.expression.texture = therapist_texture
	#### FINISH
	...
	var text_apprearing_duration: float = current_dialogue.text.length() / text_speed
	dialogue_tween.tween_property(dialogue_body, "visible_ratio", 1.0, text_apprearing_duration)	
	...
	elif prev_character != curr_character:
		dialogue_tween.tween_property(curr_character.body, "modulate", Color(1.0, 1.0, 1.0), 0.1).from(Color(0.25, 0.25, 0.25))
	...
	# Play sound effect
	var sound_max_length = curr_character.audio.stream.get_length() - text_apprearing_duration
	...
	dialogue_tween.finished.connect(func() -> void:
		...
		next_arrow.text = "v"
		var arrow_tween := create_tween().set_parallel().set_ease(Tween.EASE_OUT)
		arrow_tween.tween_property(next_arrow, "visible_ratio", 1.0, 0.1)
		arrow_tween.tween_property(next_arrow, "modulate:a", 1.0, 0.1)
